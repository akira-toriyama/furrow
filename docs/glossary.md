...
| **id** | A task's permanent handle, e.g. `t-k3m9p` — the `id` field and the stem of `bodies/<id>.md`. **Frozen**: assigned once and **never reused or renumbered**. It is a **random** value (`[ids].prefix` + a random Crockford-base32 suffix of `[ids].width` chars), generated locally with no shared counter, so concurrent `furrow add` from separate worktrees can't collide. Legacy zero-padded numeric ids (`t-0042`) stay valid and coexist. Identity is stable across moves, reorders, and renames. |
...
| **the store** | The `.furrow/` directory as a whole: `index.json` (metadata), `bodies/<id>.md` (prose), `config.toml` (human config, read-only from furrow), and `archive/` (aged done tasks). It is repo-local — created by `furrow init`, committed alongside your code, designed for clean git diffs. |
