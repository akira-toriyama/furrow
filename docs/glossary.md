...
| **lane** (== status) | A workflow column. A task's `status` field holds one lane value; the set of lanes and their top-to-bottom order is defined by `[lanes].order` in `config.toml` (default: `inbox`, `backlog`, `ready`, `in-progress`, `done`, `icebox`). "Lane" and "status" are the same thing — the field is `status`, the concept is a lane. |
...
| **terminal lane** | A lane whose tasks are *not* actionable for `furrow next` — work that is finished or parked. The set is `[lanes].terminal` (default: `done` and `icebox`). The done lane is terminal *and* stamps `closed`; a parked lane like `icebox` is terminal but does **not** stamp `closed`. |
| **actionable** | The property `furrow next` selects on: a task in a **non-terminal lane** with **all of its deps in the done lane**. The actionable set, in canonical order, is "the work ready to pick up." Computed by `core.Index.Actionable`. |
