...
| **central board** | A single `.furrow` that backs **many** repos instead of each repo owning its own store — e.g. a private cross-repo tracker. Configured **user-level** via `[[board]]` entries in `${XDG_CONFIG_HOME:-~/.config}/furrow/config.toml` (covering whole trees of repos with zero per-repo files), or **per-repo** via a `.furrow-pointer.toml`. Activating a board injects a scope **label** (often `auto` = the owning repo's dir name), which is how each task gets tagged with its repo. It is the **last** arm of discovery: a real local `.furrow` and a pointer both win over it. |
...
| **pointer** | A `.furrow-pointer.toml` at a repo root that redirects furrow to a central board (`board = <path>`) and optionally scopes the repo to one `default_label`. The per-repo counterpart to the user-level `[[board]]` config: it **wins over** the user-level boards but **loses to** a real local `.furrow`. |
| **auto-filter** | The per-board `auto_filter` switch (default `true`) declaring whether the read commands (`ls`/`next`/`revisit`) scope to the board **label**. It separates the label's two jobs: `label` is the identifier `add` tags with, `auto_filter` is whether reads are filtered by it — so `auto_filter = false` shows the whole board while writes are still tagged. A **pointer** has no such knob (it always filters). Because the choice is now explicit in config, filtering is **silent** (no scope banner). |
