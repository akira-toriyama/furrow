...
| Term | Meaning |
|---|---|
| **task** | One tracked item — furrow's unit of work. The `core.Task` struct (metadata only): `id`, `title`, `status`, `priority`, `labels`, `repos`, `parent`, `deps`, `refs`, `checklist`, `created`, `updated`, `closed`, `body`. Its long-form prose is *not* a field — it lives in a separate body file. |
...
| **repo** (the `repos` field) | A repository a task relates to, as an `owner/repo` identifier (e.g. `akira-toriyama/furrow`) in the task's `repos` array — a first-class set (sorted + deduped, `[]` never null), 0..N per task. An empty set means a **draft**: a task attached to no repo yet (the issue-draft analogue). `furrow lint` warns on an entry that is not owner/repo-shaped. |
...
| **revisit** | The read-only re-evaluation query — the agent-facing counterpart to **actionable**/`next`. `core.RevisitReasons` surfaces open (non-terminal) tasks whose metadata may be out of date, each with one or more **revisit signal** `{code, detail}` pairs: `value_unset`, `effort_unset`, `stale`, `dep_done` (one per dep already in the done lane). `furrow revisit` renders them and attaches the signals in `--json` so an agent fixes them with the setters (`value`/`effort`/`dep`); it never mutates. An empty result is healthy and exits 0 (unlike `next`). |
...
| **the store** | The `.furrow/` directory as a whole: `tasks/<id>.json` (per-task metadata shards), `meta.json` (the layout version), `bodies/<id>.md` (prose), `config.toml` (human config, read-only from furrow), and `archive/` (aged done tasks). It is repo-local — created by `furrow init`, committed alongside your code, designed for clean git diffs. |
