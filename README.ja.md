...
正準スキーマは `furrow schema [task|meta]` が出力する（draft 2020-12）。これが正本で、`docs/schema/furrow.task.v1.json` と `docs/schema/furrow.meta.v1.json` が commit 済みのコピー。CI が両者を diff して drift を防ぐ（`v1` はスキーマ**文書**の版号で、ボードのレイアウト版＝`meta.json` の `schema_version` は 2）。
