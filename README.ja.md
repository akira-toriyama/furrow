...
- **`archive`** — `<id>` 指定はそれらを名指し退避（各々 done レーン必須・違えば exit 2＝進行中を stranding しない）、id 無しは古い done を sweep（既定で全 repo、`-r`（繰り返し可）で絞る、`--older-than` で age 調整——どちらも sweep 専用で id 列との併用は exit 2）。`attach` した媒体はタスクと一緒に `.furrow/archive/` へ移動。`--yes` なしはプレビュー。
