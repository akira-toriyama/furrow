...
**furrow** は、repo の中に住むプレーンテキスト・タスクトラッカー。Go 製の単一バイナリで、構造化メタデータを 1 タスク 1 つの JSON シャード `.furrow/tasks/<id>.json` に、長文の散文を `.furrow/bodies/<id>.md` に分けて持つ。ユーザとコーディングエージェントの両方が、git で綺麗に diff できる素のテキストとしてストアを編集できることを最優先に設計している。畝（furrow）を一本ずつ進めるように、レーンを消化していく。
...
GitHub Issues は public が前提でローカルの素のテキストとは別物になりがちで、他人の issue とも混ざる。「1 ファイルに全部」型の手管理 Task.md にも独自の扱いづらさがある。
...
.furrow/
  ...
  meta.json         # ボード全体のレイアウト版（{"schema_version": 2}）
...
- **`meta.json`** = ボード全体のレイアウト版（`{"schema_version": 2}`）だけを持つ専用ファイル。**シャードの中には決して入れない**ので、版を上げても触るのは 1 ファイルだけで、どのシャードも git のマージ点にならない。
...
自動スコープできる。repo 群まとめて一度に（user-level config）か、repo ごと（pointer
ファイル）の 2 通り。
...
auto_filter = true                                    # ls/next/revisit をラベルで絞る（既定 true / false = ボード全部）
```
...
auto ラベルは付かない（stderr に注記。`-l` でスコープ指定）。`FURROW_BOARD=<path>` は単発・
テスト用に単一ボードで全体を上書きする（scope は board の repo 親）。
...
- `furrow add "…"` はスコープラベルをラベルに union（`[labels].required` も充足）。
  明示 `-l x` は置換でなく追加。
- `furrow ls|next|revisit` はスコープラベルで絞る（**banner なし＝サイレント**）。
  user-level ボードは `auto_filter = false` で「絞らずボード全部を表示（`add` のタグ付けは
  維持）」に opt-out できる。pointer は常に絞る。スコープ制御は `-r`：`-r ''` でその場限り全件、`-r <repo>` で別 repo。明示 `-l tag` はスコープ**内**をタグで絞る（AND であり、スコープを外さない）。
...
正準スキーマは `furrow schema [task|meta]` が出力する（draft 2020-12）。これが正本で、`docs/schema/furrow.task.v2.json` と `docs/schema/furrow.meta.v1.json` が commit 済みのコピー。CI が両者を diff して drift を防ぐ（`v2`/`v1` はスキーマ**文書**の版号で、ボードのレイアウト版＝`meta.json` の `schema_version` は 2）。
