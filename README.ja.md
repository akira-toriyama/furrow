...
下の表は**バイナリから生成される**: cobra ツリーの `Use`/`Short`/alias/フラグが唯一の正本で、`scripts/gen-command-table.sh` がここに流し込み、check.sh / CI はブロックとバイナリの不一致で fail する（手書きの一覧はコマンドを取りこぼし続けた——監査で 4 つ欠けていた）。ブロックは EN/JA で同一バイト——コマンド名・フラグ・一行説明は翻訳対象ではなく API 表面で、`furrow <cmd> --help` と同じもの。日本語の深掘りは表の外の[コマンド補足](#コマンド補足)と各節にある。
...
- **`ls`** — 正準順（`lane -> priority -> id`）で一覧。`--drafts` は repo 未付与のタスクだけ（ボードのスコープは無視）。`--since`/`--until` は `updated` で期間フィルタ（素の `YYYY-MM-DD` または RFC3339。素の `--until` はその日を丸ごと含む）。`--sort updated|created|value|effort` で並べ替え（新しい/大きい順、`--reverse` で反転、未設定の見積もりはどちら向きでも末尾）、`--sort` 時は `-n` がソート後の上位 N。未知の `--sort` フィールド・不正な日付は exit 2。`--archived` は archive store から同じフィルタ/sort で一覧。フラットな各行は 1 文字の**状態記号**を持つ——★ 着手可能（next レーンかつ依存が全部 done＝まさに `furrow next` が渡すもの）・✓ done・~ 保留・▣ container の箱・· 着手不可——`--json`/`--ndjson` は各行に `actionable`・`blocked_by`・`container`・`stuck` を付ける。状態で絞るには `--actionable`（★ だけ）か `--blocked`（依存が満たされていない行だけ）——排他で、どちらも `-s/-l/-r` と AND（`-s ready --blocked` = ready なのに実は詰まっている行）。**`--tree`** は同じ事実を **parent の階層**として描く——top-level タスクごとに 1 本の木、`<id>` を渡せばその部分木。フィルタはそのまま効き、森は**マッチした集合の上に**組む——親がフィルタで落ちた子は消えずに root になる（`--tree` で見えるタスクが減ることは無い）。`--tree` 時の `-n` は**木の本数**の上限（タスク数ではない）。`--tree` では `--json` が `children` を入れ子にし、`--ndjson` は 1 行 1 本の木。**container**（epic）のノードは子の進捗ロールアップ（`progress` = `done/total`、既定は直下の子・`--progress-recursive` で部分木全体）と `stuck`（配下に open な仕事があるが actionable な子孫が無い）も持つ。`--type` は effective 型で照合（`--type task` は型無しの多数派も含む）。**`-q "<query>"`** は GitHub Projects 風の型付きクエリを1本の文字列に畳む——**フラットな AND リスト**（項の間の空白＝AND、値の中のカンマ＝OR、先頭 `-`＝NOT）で、他フィルタ（`-s/-l/-r` 等）と AND 合成するのでスコープを広げない。cross-field OR も grouping も in-query sort も無し（GitHub 自身の天井。長い尾は `--json | jq`）。qualifier（`field:value`）= `status`/`lane`・`type`・`label`・`repo`・`id`(前方一致)・`parent`・`title`(部分一致)；序数（`value`/`effort`/`priority`/`roi`）は比較 `>=,<` と範囲 `2..4`/`*..3`/`3..*`（未設定見積もり・未定義 roi は比較を満たさない）；`has:`/`no:`（`no:repo`＝draft・`no:value` 等）；furrow 独自の計算フラグ `is:actionable`(まさに `furrow next` が渡すもの)/`is:blocked`/`is:stuck`/`is:open`/`is:closed`/`is:draft`/`is:container`；素の語・`"引用句"` は title の部分一致。未知の `status`/`type` 値・未知 qualifier・非序数への比較演算子は exit 2＋安定 `id`＋`candidates`（黙って空を返さない）。今は `ls` に配線済み——`next`/`revisit` への拡張、日付 qualifier（`updated:>=-2w`）・依存グラフ qualifier（`child-of:`/`depends-on:`）は次段。例: `furrow ls -q 'is:actionable label:cli,dx -status:icebox'`。
- **`show`** — 任意個の id を 1 回の読みで、入力順に表示。複数 id は `--json` で配列・human は `---` 区切り（1 id は従来の単一オブジェクト、`--ndjson` は個数によらず 1 行 1 タスク）。`--no-body` は本文（`body_text`）を省く＝軽量なメタデータ一括読み。一部 id が無くても見つかった分は出力し exit 1、エラーに `details.missing`。見つからない id が **archive 済み**なら `details.archived` にも載り、メッセージが `--archived` での再試行を促す。`--backlinks` は本文でこのタスクを `[[id]]` 参照する他タスクを列挙（`--json` では `mentioned_by` 配列。`--archived` とは併用不可）。
- **`next`** — 「着手可能」= レーンが `[next].lanes`（既定 `ready` + `in-progress`、intake は出ない）にあり**かつ**依存が全部 done。**container**（epic）は箱であって仕事ではないので出さない——`--containers` で着手可能な箱も出す。`--lanes <csv>` はこの呼び出しだけ「今」とみなすレーンを上書き（config は不変・非破壊）——`next --lanes backlog,ready` は昇格前でも今すぐやれる無依存の backlog タスクを出す。未知レーンは exit 2＋`candidates`。`--json` は各タスクに `reason`（`in_next_lane`＝マッチしたレーン名・`deps_satisfied`）を付与。
- **`brief`** — セッション開始のワンショット read: `next` の上位 `-n` 件（既定 3）を **body 付き**で（`body_text`——後追いの `show` を畳み込み）＋ `next_total`（cap 前の着手可能総数——cap が待ち行列の大きさを隠さない）／`blocked`（next レーンに居るが依存未達のタスク＋`blocked_by`——素の `next` が意図的に隠す「着手済み・詰まり中」の仕事）／`revisit` 要約（`sync` と同形）／`drafts` 数（定義上ボード全体——draft は repo を持たないのでどのスコープにも属さない）。`-r`/`-l` で他の read と同様にスコープ。human モードは body なしのコンパクトなダッシュボード（散文は agent 向け `--json` の積み荷）。read-only で git に触れない——共有ボードでは `furrow sync && furrow brief` で現在地を掴む。
- **`revisit`** — read-only。`--json` は各タスクに `revisit` 配列 `{code, detail}`（`no_repo`・`value_unset`・`effort_unset`・`stale`・`dep_done`）を付与し、エージェントが何を直すか分かる。draft はスコープに関係なく浮上。`--stale-days 0` で stale 無効。
...
- **`repo`** — 値は完全な `owner/repo` か、ボード既知の repo に一意解決する短名のみ（それ以外は exit 2＋`candidates`）。repos が空のタスクは draft。
- **`review`** — id 形の引数はそのタスクの `reviewed` を打刻（`updated` とは別管理＝レビューは内容を変えない）、それ以外は repo 単位のレビュー時計を記録。`--by human`（既定）は staleness nudge の時計を進め、`--by agent` は sweep を記録するが人間の時計は進めない（自律再評価が人間への nudge を止めない）。
- **`apply`** — PR/コミット本文の `SetStatus-task: <body-link> [<lane>]` ディレクティブを解析して適用（stdin または `--body-file`）——[status 自動更新](#ci-pr-から-tracker-を自動更新)の CI フック。`--on open` は in-progress へ寄せ、`--on merge` は lane を適用。検証は非ブロッキング。
- **`archive`** — `<id>` 指定はそれらを名指し退避（各々 done レーン必須・違えば exit 2＝進行中を stranding しない）、id 無しは古い done を sweep（**全 read と同じくボードスコープを継承**＝ls/next が見せていた repo だけを畳む。`-r`（繰り返し可）でスコープを切り替え、`-r ''` でボード全体＝明示タイプが要る。最大の破壊半径に最多キーストロークを課す）、`--older-than` で age 調整——どちらも sweep 専用で id 列との併用は exit 2）。`attach` した媒体はタスクと一緒に `.furrow/archive/` へ移動。`--yes` なしはプレビュー。
- **`lint`** — 各 finding は安定した kebab-case の `code`（`dangling-link`・`dep-cycle`・`parent-cycle`・`conflict-marker`・`unknown-shard-key`・`schema-outdated`・`archive-backlog` …）を持つ——message でなく code で分岐する（`id` は文脈依存: task id・asset 名・`owner/repo`・`meta`・`config`）。error: 依存/階層の循環（親の循環は root を失わせる）・closed 無しの done タスク・body の git conflict marker（半分マージされた進捗記録。``` フェンス内は解説なので対象外）。warn: done な親の下の open タスク・dangling `[[id]]`（archive 済み id は対象外）・reconcile gap・アセット衛生（missing/orphan/5 MiB 以上）・バイナリより古いレイアウト・furrow が知らないキーを持つファイル・config clamp・`[lint].archive_done` の backlog nudge。`--code`（許可リスト）/`--exclude-code`（除外・勝つ）/`--severity error|warn` で絞る——未知の `--code` トークンは exit 2＋`candidates`、config の `[lint].ignore_codes` の未知エントリは warn だけ（clamp-don't-reject）。**フィルタが exit code を決める**: 絞り落とした problem は見つからなかったのと同じで、最後の error を除外すれば exit 0（恒久的に死んだ検査を黙らせて CI を赤くしない）、`--severity warn` は常に exit 0。
- **`migrate`** — dry-run が既定（`--write` で適用）。未対応の見出しや `[[wikilink]]` は破棄せず報告。
