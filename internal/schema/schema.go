...
const TaskV2 = `{
  ...
  "properties": {
    ...
    "repeat": { "type": "string", "description": "the recurrence rule this task runs on: ONE RFC 5545 RRULE line, with no DTSTART line (repeat_anchor carries the series start); absent = not repeating. Closing a repeating task generates its successor and MOVES this field (and repeat_anchor) to it, so the rule is held by exactly one task at a time — re-closing therefore cannot mint a second successor." },
    "repeat_anchor": { "type": "string", "format": "date-time", "description": "the series start the rule is expanded from: the FIRST occurrence's due. Immutable for the life of the series and inherited unchanged by each successor, which is what lets COUNT terminate and keeps a snooze (set --due) from re-latticing later occurrences. Present iff repeat is." },
...
// v7 shape: the required deps set (the epic ids this box waits on) plus the
// required standing/pinned declarations. v1 is retired,
...
const EpicV2 = `{
  ...
  "description": "Schema for one .furrow/epics/<id>.json shard (a single epic: a box of work with member tasks). The board-wide schema_version lives in .furrow/meta.json, never in a shard. v2 is the board-layout v7 shape: the required deps set (the epic ids this box waits on — information, not enforcement; epic activate warns about an open dep and proceeds) plus the required standing/pinned declarations. Pin to a tagged URL or vendor this file.",
  "type": "object",
  "additionalProperties": true,
  ...
  "required": ["id", "title", "goal", "active", "labels", "repos", "meta", "created", "updated", "closed", "body", "deps", "standing", "pinned"],
  "properties": {
    ...
    "standing": { "type": "boolean", "description": "declares a PERMANENT box (an inbox-like channel, a parking lot): exempt from the lifecycle revisit nags epic_all_done, epic_dep_done, and epic_stuck (its replacement signal is the review cadence, epic_review_due). furrow never interprets which boxes deserve it — that binding is the operator's convention." },
    "pinned": { "type": "boolean", "description": "the box's actionable tasks surface in next/brief REGARDLESS of the active-epic scope, at the top — an always-visible channel. Orthogonal to active and standing (a mandate box is standing+pinned; a parking lot standing only); needs no repo slot, the read's own repo filter scopes the tasks." },
    "reviewed": { "type": "string", "format": "date-time", "description": "when a human last re-assessed this box's contents (furrow review <epic-ref>, board layout v9); absent = never (which is why it is not in required). Tracked separately from updated (a review changes no content). Read by revisit's epic_review_due: a STANDING box whose last review is past [review].stale_after_days — the same clock as the per-repo review nudge — is due; a never-reviewed box stays quiet (the first review opts it in)." }
  }
}
`
