...
func TestReadOnlyBoardNotesOnReads(t *testing.T) {
	...
	for _, args := range [][]string{{"ls"}, {"brief"}, {"show", "--no-body"}, {"next"}} {
		...
		stdout, se, code := runSplit(t, args...)
		if code != 0 {
			t.Fatalf("%v on a read-only board: exit %d", args, code)
		}
		if !strings.Contains(se, "READ-ONLY") || !strings.Contains(se, "v8") {
			t.Errorf("%v: stderr %q must carry the READ-ONLY note with the versions", args, se)
		}
		// Both ways out, not just the flag day: running a furrow at the
		// board's layer is the usual fix on a shared board (2026-08-12).
		if !strings.Contains(se, "board's layer") || !strings.Contains(se, "furrow upgrade") {
			t.Errorf("%v: stderr %q must name both remedies", args, se)
		}
	...
	_, se, code := runSplit(t, "board")
	if code != 0 {
		t.Fatal("board must still answer")
	}
	if strings.Contains(se, "READ-ONLY") {
		t.Errorf("board: the note must not fire on the reporting command (stderr %q)", se)
	}
}
...
func TestTooNewBoardReadFailsWithoutNote(t *testing.T) {
	...
	_, se, code := runSplit(t, "ls")
	if code != 3 {
		t.Fatalf("a too-new board's read must refuse with exit 3, got %d", code)
	}
	if strings.Contains(se, "READ-ONLY") {
		t.Errorf("the READ-ONLY note must not front a refused read: stderr %q", se)
	}
}
