...
// EpicIDPattern is IDPattern's twin for epic ids ([ids].epic_prefix + base32).
// Separate rather than one pattern over both prefixes: a task id that landed in
// an epic shard (or the reverse) is exactly the mix-up worth catching, and one
// permissive pattern would call it fine.
func (c *Config) EpicIDPattern() *regexp.Regexp { return c.epicIDPattern }

func (c *Config) compile() {
	c.idPattern = regexp.MustCompile("^" + regexp.QuoteMeta(c.IDPrefix) + `[0-9a-z]+$`)
	c.epicIDPattern = regexp.MustCompile("^" + regexp.QuoteMeta(c.EpicIDPrefix) + `[0-9a-z]+$`)
