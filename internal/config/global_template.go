...
# Label that ` + "`furrow add`" + ` tags new tasks with: "auto" = the nearest enclosing git
# repo's directory name, "" = no label, or a literal string.
label = "auto"
# Auto-filter reads (ls/next/revisit) by that label; false shows the whole board
# while ` + "`add`" + ` still tags with the label.
auto_filter = true
