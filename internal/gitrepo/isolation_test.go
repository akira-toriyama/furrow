...
func TestEmptyCloneLandsOnMain(t *testing.T) {
	...
	gittest.RunGit(t, git, filepath.Dir(clone), "clone", "-q", origin, clone)
	...
	if got := strings.TrimSpace(gittest.RunGit(t, git, clone, "symbolic-ref", "--short", "HEAD")); got != "main" {
	...
	if got := strings.TrimSpace(gittest.RunGit(t, git, clone, "config", "--get", "init.defaultBranch")); got != "main" {
	...
	if got := strings.TrimSpace(gittest.RunGit(t, git, clone, "config", "--get", "commit.gpgsign")); got != "false" {
