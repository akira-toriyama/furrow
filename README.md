...
- **`archive`** — with ids it retires exactly those (each must be in the done lane, else exit 2 — no stranding live work); with none it sweeps aged done **inheriting the board scope, like every read** (`-r` (repeatable) swaps that scope; `-r ''` sweeps the whole board and has to be typed — the widest blast radius costs the most keystrokes, not the fewest), `--older-than` adjusting the age guard (sweep-only flags; combining them with ids is exit 2). A task's attached media travels with it into `.furrow/archive/`, never orphaned in the hot store. Previews unless `--yes`. Archiving is a **round trip** — see `unarchive`.
...
jobs:
  sync:
    uses: akira-toriyama/furrow/.github/workflows/sync-task-status.yml@v4.0.0
    secrets:
