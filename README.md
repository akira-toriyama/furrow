...
| `ls` (alias `list`) | List tasks in canonical `lane -> priority -> id` order; `--drafts` lists only the tasks with no repo (bypasses the board scope). `--since`/`--until` window by the `updated` timestamp (a bare `YYYY-MM-DD`, or a full RFC3339 instant; a bare `--until` includes the whole day). `--sort updated\|created\|value\|effort` reorders (newest/highest first; `--reverse` flips it, an unset `value`/`effort` stays last either way); with `--sort`, `-n` takes the top N of the sorted set. An unknown `--sort` field or an invalid date is exit 2. `--archived` lists from the archive store (`.furrow/archive/`) instead of the hot board (the same filters/sort apply). Every flat row carries a one-character **state glyph** (★ actionable — a next lane with every dep done, exactly what `furrow next` would hand you; ✓ done; ~ parked; ▣ a container box; · open but not available), and `--json`/`--ndjson` add `actionable`, `blocked_by`, `container`, and `stuck` to every row (the derived facts that were `--tree`-only before). Filter on the state with **`--actionable`** (only ★) or **`--blocked`** (only rows with an unsatisfied dependency); both AND with `-s/-l/-r` — `-s ready --blocked` is the ready rows that are actually stuck — and are mutually exclusive (a task can't be both). **`--tree`** draws the same facts as the parent hierarchy instead of a flat table — one tree per top-level task, or the subtree under `<id>` when given (`furrow ls --tree <id>`). Every filter still applies and the forest is built over what matched, so a task whose parent was filtered out becomes a root rather than disappearing: `--tree` never shows fewer tasks than the same flags without it. With `--tree`, `-n` caps the number of **trees**, never the tasks. Under `--tree`, `--json` nests children (the same `actionable` + `blocked_by` per node) and `--ndjson` streams one whole tree per line. For a **container** (an epic) the tree also carries rolled-up child `progress` (`done/total`, direct children by default, whole subtree with `--progress-recursive`) and a `stuck` flag (open work under it but no actionable descendant). `--type` filters by work-item type (by effective type, so `--type task` includes the type-less majority) | `-s/--status`, `-l/--label`, `-r/--repo`, `-n/--limit`, `--drafts`, `--actionable`, `--blocked`, `--since`, `--until`, `--sort`, `--reverse`, `--archived`, `--tree`, `--type`, `--progress-recursive` |
...
| `next` | Show actionable tasks (lane in the configured `[next].lanes` — default `ready` + `in-progress`, so intake lanes stay out — and all deps done). A **container** type (an epic) is a box, not work, so it is skipped; `--containers` surfaces a ready one too. **`--lanes <csv>`** overrides which lanes count as "now" for this call only, leaving `[next].lanes` in config untouched (non-destructive) — `next --lanes backlog,ready` surfaces a no-dependency backlog task you could start now without first promoting it; the deps-done half is unchanged, and an unknown lane is exit 2 with the configured lanes in `candidates` (like `-s`). `--json`/`--ndjson` attach a `reason` (`in_next_lane` — the lane each task matched, so a `--lanes`-included one is distinguishable — and `deps_satisfied`) | `-l/--label`, `-r/--repo`, `-n/--limit` (use `-n1` for just the top), `--containers`, `--lanes` |
...
| `value <id> <1-5>` | Set a task's coarse value (importance) estimate; an out-of-range score clamps to 1..5 **and is signaled** — a `clamped` key nested by field (`clamped.value.{requested, stored}`) in the `--json` mutation envelope plus a stderr note (so an explicit arg is never silently rounded); `--clear` unsets | `--clear` |
...
| `schema [task\|meta\|repo]` | Print the JSON Schema for a task shard (no arg or `task`), for `meta.json` (`meta`), or for a `repos/` review shard (`repo`); each matches its committed copy | — |
