...
| `add <title>...` | Add a task (or many with --stdin) | `--body`, `--check`, `--dep`, `--draft`, `--due`, `--effort`, `-e/--epic`, `-l/--label`, `-p/--priority`, `--ref`, `--repeat`, `-r/--repo`, `-s/--status`, `--stdin`, `--value` |
...
- **`add`** — creates one task per stdin line with `--stdin`; `--check` (repeatable) seeds checklist items (body prose alone never populates the shard checklist); an out-of-range `--value`/`--effort` clamps to 1..5 with a stderr note; a title starting with `-` needs a `--` separator (the error says so); `-e/--epic` files the task under a box (id, unique id prefix, or unique title substring; a miss or ambiguity is exit 2 with `candidates` — leaving it unfiled is legal at add time and an error in `furrow lint` while the task is open); `--due` promises the task for a date — `2026-08-04` (that WHOLE day: it binds 23:59:59 in your zone, so the day itself never starts out overdue), `2026-08-04T10:30`, an RFC3339 instant, or a signed offset like `+1d`. **`--repeat` makes the task RECUR** (board layout v10): give it a short spelling — the closed grammar `furrow add --help` lists: `daily`, `every <n> days`, `weekly`, `weekly on <days>`, `every <n> weeks on <days>`, `monthly`, `monthly on <day-of-month>`, `monthly on last`, `monthly on <nth> <weekday>`, `monthly on last <weekday>`, `every <n> months on <day-of-month>`, `yearly`, `every <n> years` (so `every 2 weeks on mon,thu`, `monthly on 2nd tue`, `monthly on last fri`, `every 3 months on 15`), optionally ending in `until <date>` or `for <n> times` (never both — RFC 5545 forbids it) — or a raw RRULE line for anything the short grammar cannot say, **minus a `DTSTART`**, which is exit 2 in either spelling (a `DTSTART=` term or a leading `DTSTART:`/`DTSTART;TZID=…` line): furrow's series start is the `--due`, so a DTSTART could only be a value it took and then ignored. It is compiled to one RFC 5545 RRULE — a weekday list is a **set**, so naming a day twice (`weekly on mon,mon`) stores it once, in the order you typed — and needs a **`--due`**: that first date becomes the immovable series anchor. **The anchor need not be a date the rule LANDS on** — `--due <a Friday> --repeat 'weekly on mon'` is legal — and furrow never folds an off-lattice one in: RFC 5545 leaves a start not synchronized with its rule undefined (§3.8.5.3), while the model §3.3.10 teaches for COUNT is the opposite one ("the DTSTART property value always counts as the first occurrence"). furrow's answer is that such an anchor is one live occurrence **outside** the series, so `for <n> times` counts the rule's own lattice **slots** and hands out n occurrences AFTER it: `--due <a Friday> --repeat 'weekly on mon for 3 times'` is **four** tasks, not three, where the same rule anchored on a Monday is three. The refusal that catches `for 1 times` on a lattice day ("no occurrence after the first one — it would end the series on the very next close") does not fire off it, either. A stderr note says so at bind time and names the rule's own first date; anchor there to fold the first occurrence into the series. A rule naming a day past 28 (`monthly on 31`) is legal and SKIPS the months that lack it, per RFC 5545 — a stderr note says so at bind time and points at `monthly on last`, the rule that always lands. February 29 is the one day a whole YEAR can lack, so a rule landing there (`yearly` or `every <n> years` anchored on it, or a line naming `BYMONTH=2;BYMONTHDAY=29`) gets its own note, carrying the real next occurrence (anchored 2028-02-29, a `yearly` rule next lands 2032-02-29) and pointing at February's last day (`BYMONTH=2;BYMONTHDAY=-1`) — never at `monthly on last`, which is a rule of another frequency.
- **`ls`** — lists in canonical `lane -> priority -> id` order. `--drafts` shows only repo-less tasks (bypasses the board scope). `--since`/`--until` window by `updated` (bare `YYYY-MM-DD` or full RFC3339; a bare `--until` includes the whole day); `--sort updated|created|value|effort` reorders (newest/highest first, `--reverse` flips, unset estimates stay last either way) and makes `-n` the top-N of the sorted set; an unknown `--sort` field or bad date is exit 2. `--archived` reads the archive store with the same filters. Every flat row carries a one-character **state glyph** — ★ actionable (a next lane, every dep done — ready to pick up; `furrow next` additionally scopes to the active epic, so ★ is a superset of what next hands you), ✓ done, ~ parked, · open but not available — and `--json`/`--ndjson` add `actionable` and `blocked_by` per row. Filter on the state with `--actionable` or `--blocked` (mutually exclusive; both AND with `-s/-l/-r`, so `-s ready --blocked` is the ready rows that are actually stuck), and on box membership with `-e <epic>` (strict — the unfiled pile is `-q no:epic`, not a box name). **`--tree`** groups the same rows by **epic** instead of a flat table: the active epic first, then open epics by id, then closed ones, then the unfiled group — or one box's group with an `<epic>` argument. Filters still apply and the grouping is built over what MATCHED, so `--tree` never shows fewer tasks than the same flags without it; `-n` caps the number of **groups**, not tasks. Each group carries the epic's member `progress` (`{done,total}`, counted over the FULL board so a read filter can't under-count the box) and a `stuck` flag (open members, none actionable). Under `--tree`, `--json` nests the member tasks per group and `--ndjson` streams one whole group per line. A dated row carries its promise in the title cell — `due 2026-08-04 10:30`, or `overdue …` once the instant has passed (a tag, not a column: only a few tasks carry a date, and a column would widen every board). A **repeating** row is tagged the same way, with a bare `repeats` beside the date, because closing it MINTS the next occurrence — a row that costs you a write must not render like a one-off. That tag rides **wider** than the date does: every human view that renders a task *as* a task carries it (`ls` flat and `--tree`, `next`, `revisit`, all three of `brief`'s bands, `epic show`'s members), since a date is guaranteed a surface (brief's due band, `lint` board-wide) while a rule has none — a chore promised for next month reaches you only as an ordinary row. It is a fixed word, never the rule: the shard stores the compiled RRULE and `furrow show` is where that belongs. (`search` is the one exception — its `MATCH` column is a snippet, not a title cell.)
...
- **`next`** — "actionable" means: lane is in `[next].lanes` (default `ready` + `in-progress`, so intake stays out) **and** every dep is done. On a board with at least one epic, the result is ALSO scoped to the **active** epic for the repo, plus the unfiled pile — a **pinned** box's actionable tasks pass through that scope entirely and LEAD the result (v7: the always-visible channel), then the focus box's tasks, then the unfiled rescue; with no active epic the result is deliberately empty except for the pinned band (exit 0, a stderr hint names the state either way). `-e <epic>` reads one box explicitly (strict) and `--all-epics` ignores the scope; a board with no epics behaves classically. `--lanes <csv>` overrides which lanes count as "now" for this call only (config untouched): `next --lanes backlog,ready` surfaces a no-dependency backlog task without first promoting it; an unknown lane is exit 2 with `candidates`. `--json` attaches a `reason` per task (`in_next_lane` — the lane it matched — and `deps_satisfied`). A **due** promise that has arrived is noted on **stderr** (`note: N due (M OVERDUE)`), never folded into the rows: the dated work usually sits in a lane `next` excludes, so it has to be visible without changing what "actionable" means — and stdout stays a clean array. A **repeating** row, by contrast, is tagged in the row itself (`repeats`, see `ls`): it is not another list to reconcile but a property of the pick in front of you — closing it writes another task.
- **`brief`** — the one-shot session-orient read, and the place a **due** date lands: the section LEADS the output (`due`, `{overdue, today}`, longest-overdue first) because a date is the only thing on the board that expires — everything else brief reports waits for you. **No automatic scope narrows it**: not the epic focus (promised work is usually parked outside it), not the lane filter, and not the board's repo scope — that one is derived from the cwd rather than typed, and while it applied, a task promised for today was invisible at session start whenever it happened to name another repo, even though `lint` (which has no repo filter at all) was erroring on it, i.e. brief printed a due section of 1 above a lint ride-along counting 2. An **explicitly typed** `-r`/`-l` still narrows it — the reader chose that one and can drop it — so the rule is about who applied the filter, not which field it is. The key is omitted entirely when nothing is due. `lint`'s `due-overdue`/`due-today` are its twin and now count the same set — this is the surface a session cannot miss, that one is the check a sweep goes looking for. Then the **epic header** (`active`, the open+active epic(s) `next` scopes to with their member roll-up; `pinned`, the open pinned channel(s) whose tasks lead `next`, deduped against active — only channels with an **open member** are listed, the quiet rest (a mandate inbox's healthy empty state) collapsing to a `pinned_quiet` count so a fleet of standing boxes is one line, never hidden silently; and `epics_declared`, which tells a non-participating board apart from "nothing active, so `next` is deliberately empty"); `next`'s top `-n` picks (default 3) **with their bodies** (`body_text` — the `show` follow-up folded in) plus `next_total`, the uncapped actionable count (a cap never hides the queue size); `blocked`, the next-lane tasks with an unsatisfied dep and their `blocked_by` (started or queued work that plain `next` deliberately hides); the `revisit` summary in `sync`'s shape; the `drafts` count (board-wide by definition — a draft has no repo, so no scope can own it); and `sync`'s `lint` error-count ride-along (omitted when clean). Scope with `-r`/`-l` like every read. All three task bands (due, next, blocked) tag a **repeating** row with `repeats` — wider than the date rides, which lands in the due band alone: a rule has no band of its own, and the next band is exactly the row a session closes. Human mode is a compact dashboard **without** bodies (prose is `--json`'s payload for agents). Read-only and git-free: orient on a shared board with `furrow sync && furrow brief`.
- **`revisit`** — read-only; `--json` attaches a `revisit` array of `{code, detail}` (`no_repo`, `value_unset`, `effort_unset`, `stale`, `dep_done`) so an agent knows what to fix. The box-level half of the vocabulary — `epic_all_done` / `epic_stuck` / `epic_stale` / `epic_dep_done` / `epic_review_due` — is about epics, not tasks, and surfaces on `sync`/`brief`'s revisit summary keyed by epic id (this command's rows stay task-shaped). Drafts surface regardless of scope. `--stale-days 0` disables the stale signal.
...
- **`doctor`** — `boards`' opinionated sibling: the machine-wide board-setup **health check**, read-only and network-free (it never fetches). The board the invocation itself resolves to — `FURROW_DIR`, a repo-local `.furrow`, a pointer, or a `[[board]]` scope — is probed **first**, whether or not a `[[board]]` names it, and every finding's id names its board. It checks that the user config parses and at least one usable `[[board]]` exists (`no-boards` — the classic half-set-up machine, where every use is a bare exit 2 naming nothing), that every board is on disk, readable, and on this binary's schema, that every scope directory exists, a git-backed board's freshness vs its upstream *as of the last fetch* (`board-behind` → sync before reading; `board-ahead` → unpushed writes; an in-progress rebase/merge warns too), and where discovery would **not** pick the board inside its own scopes (`scope-shadowed`: a nearer `.furrow`/pointer wins — severity `info`, a fact to see, never unhealthy; when the shadow is the board's OWN tree, a bare `add` there also warns at the moment it drafts, so the finding is no longer doctor-only). Discovery is simulated at cwd (informational) and at every dir passed as an argument (an **assertion**: `dir-unresolved` is an error carrying the fix). It also probes the Claude Code session registry the session write guard reads (the `session-busy` bullet above), so a registry that no longer parses — the shape a format change takes, under which the guard silently protects nothing — is a `session-registry-unreadable` warning. Every finding has a stable kebab-case `code`; exit `0` = healthy (info included), `1` = problems found (`doctor-unhealthy`), so it can sit in shell init or CI: `furrow doctor --json | jq -e '.healthy'`.
- **`edit`** — opens `bodies/<id>.md` in the editor; with no TTY it prints the path instead. `--body "<markdown>"` skips the editor: it **replaces the whole body and advances `updated`** in one write — the non-interactive edit a front-end's $EDITOR round-trip needs, where a direct write to the printed path would leave `updated` stale. `-` reads the new body from stdin (the shared `-`=stdin convention); an empty replacement is exit 2, never a silent clear, and the `--json` envelope carries `replaced_bytes` (the body is unbounded, so the effect is counted, not echoed). `<id>` may name a **task or an epic** (same membership routing as `note` — both entities' prose is one file in one directory). Prefer `note` when you mean to *append* a progress record: a direct file edit does not advance `updated`.
...
- **`done`** — **this is what advances a recurring task.** Closing an occurrence writes the next one in the same all-or-nothing write and hands the rule over to it, so a series is carried by exactly ONE live task: closing twice, or reopening and closing again, mints nothing. The guarantee rests on the predecessor having given the rule up, which only a hand-edited shard can undo — `lint` errors `repeat-on-closed` when a done-lane task still carries one (that state would mint a second successor on the next reopen-then-close, and it also hides the task from `repeat-no-timezone`, which counts live carriers). The successor is born in the **default lane** with the body and checklist copied (every box unchecked) under a `previous: [[id]]` line (marked, so a leading link of your own is never overwritten) — the body as it stood just BEFORE this close's `--note`, so the closing word stays with the occurrence that earned it while everything else written during the cycle (a mid-cycle `furrow note`, an `apply` annotation) carries forward as part of the chore's prose — inheriting title, labels, repos, epic, value, effort and refs — everything except what the close settled (`closed`, `reviewed`), what the rule computes (`due`), this run's `deps`, and its position: priority is lane-relative, so the successor is **appended** to the default lane exactly as `add` appends (a copied number tied whatever already sat there). **The `epic` comes over whatever state the box is in**, including a CLOSED one — dropping it would trade `lint`'s `epic-closed` warn for an `epic-required` error — so a series filed under a closed box re-raises `epic-closed` every cycle (the warn MOVES to the new id, it does not pile up). `furrow epic done` discloses that at close time (see `epic` below), and one `furrow set <live-id> -e <open-epic>` carries the whole series out: every later occurrence inherits the new box. Its due is the first occurrence after what the close **settles**: for a bare-date series (a `23:59:59` anchor) the **whole local day** of the later of now and the due — the day the work was done, or the day it was promised for while that is still ahead — and for a timed one (`--due …T21:00`) the later of now and the due as instants, so tonight's 21:00 stands after a 17:00 close. An on-time or early close therefore advances exactly one step, a snoozed one (`set --due +1d` lands off-lattice) never hands the same day back, a late close jumps past the cycles that lapsed instead of building a backlog you never accrued, saying how many (what lies strictly between the settled occurrence and the day of the close): `t-k3m9p  repeat: next due 2026-06-01 23:59 +09:00 (t-p4q2r) — 2 occurrence(s) skipped`. The receipt is prefixed with the id of the occurrence that **closed** — the same `<id>  <line>` shape `apply` prints, at every arity — because a batch close prints one verb line per task but only as many receipts as there were series: position would map a mixed batch's lone receipt to the task that does not repeat. And a chain of afternoon closes is on time again after **one** late day, instead of late forever. Two flip sides are deliberate: a close just past midnight settles the **new** day, so closing yesterday's daily at 00:10 consumes today's (re-date the successor with `furrow set <id> --due <today>` when that is not what you meant), and `for <n> times`/`until` count lattice **slots** — the same counting that leaves an off-lattice anchor outside the series (see `add`) — so a close that lands on a later slot's day settles that slot too and can spend a bounded series a close early. When the rule is spent it says `t-k3m9p  repeat: series complete — no further occurrences`, with the same lapse count when a late close ran it out. `--json` carries the same facts on each envelope under `repeat` (`{created, due, skipped, completed}` — always that shape, so "does not repeat" (no key) and "this was the last one" (`completed: true`) stay distinguishable). Nothing is time-driven: see [non-goals](docs/non-goals.md). The series is expanded in the board's `[due].timezone`; undeclared, that is the zone of whichever machine closes the occurrence — on a shared board that includes the UTC CI runner behind `apply`, which puts a bare-date anchor on a different calendar day than the operator's zone (every bare date west of UTC) and so shifts a `weekly`/`monthly` occurrence by a day — and the two disagree about which day a close settles. Occurrences keep their wall clock in that calendar across DST, including the one day a year a zone has no local **midnight** at all (America/Santiago, America/Havana and Atlantic/Azores spring forward *at* 00:00): the series is expanded in a frame with no such gap and read back into the board's zone, so that day gets its occurrence and a `weekly on sun` chore is never promised for the Saturday. `lint` errors `repeat-no-timezone` on a shared board carrying a live rule with no declared zone; declare it with `furrow config set due.timezone <IANA zone>`. `--note "<text>"` folds the closing word ("→ continued in t-xxx") into the close itself: the text lands on **every** closed task's body under the note command's contract (a new paragraph, `updated` advances, `appended` beside each `--json` envelope, `-` reads stdin), and an empty note is exit 2 — never a silent plain close.
...
- **The session write guard** (every task and epic write, under Claude Code only) — the answer to the co-located-session incident: one Claude Code session chatting with the human ran `furrow add` into a repo where ANOTHER session was working autonomously, having judged that "creating a task is not interference". A write that touches a repo an **earlier-started** Claude Code session on this machine is sitting in is **refused** — exit 2, kind `session-busy`, every clash in `details.clashes` (`repo`, `pid`, `session_id`, `name`, `cwd`, `started_at`, `last_active`, `idle_seconds`, `turn_ended`, `busy`) — while that session is **working**, and goes **through with a warning** once it is **idle**. Idle is read off the occupant's transcript: its last message record is an assistant `end_turn` (the session finished its turn and is waiting for the human — however fresh that write is, since the closing message IS the last write; `turn_ended: true` in the clash), or, when the turn is not known to have ended (mid-turn, or a tail the adapter cannot read), it has been silent longer than `[session].busy_seconds` (default 300 — the ceiling that keeps a long tool call or an unanswered permission prompt from refusing forever; a session whose transcript cannot be found is measured from its start under the same ceiling — never busy forever; `0` turns refusals off and warns instead). The warning is one stderr line plus a `session_warn` `{clashes}` key in the `--json` envelope (on every envelope of a batch write; `add` prints the created task, so its warning is stderr-only). The repos judged are the entity's: a new task's after the board-scope union (so a bare `add` inside the occupied checkout is caught), an existing task's or box's **before and after** the edit (`--add-repo`/`--rm-repo` are judged on both sides). **First come, first served** — the earlier session's own writes never clash, so an autonomous loop is never stopped by the session that came to watch it. The escape is not a new flag: `add --draft` creates the task repo-less (attach later with `furrow repo <id> --add`), and `details.hint` names it; for any other write, wait for the session, hand the write to it, or run from a shell outside Claude Code — a **human shell and CI pass untouched** (no `CLAUDECODE` env). Best-effort by contract, since the registry is Claude Code's private format: when it cannot be read, this session is not in it, or this session's own transcript cannot be found (the sign the transcript derivation is wrong, under which every occupant would read as busy), the guard **stands down and says so** on stderr (`note: session guard: … standing down`) rather than refuse on a guess; `furrow doctor` reports a registry that no longer parses as `session-registry-unreadable`. Not guarded: reads, board maintenance (`archive`, `tidy`, `upgrade`, `review`'s stamps, `sync`).
...
- **`epic`** — the box's own subcommands: `add` (never active at birth — opening a box is a separate, deliberate act), `ls` (active first, then open by id, then closed; `--all` includes closed; the board's repo scope applies exactly as on the task reads — a bare `epic ls` inside a scoped checkout lists this repo's boxes, the population `brief`'s epic header draws from, with a stderr note for what the scope hid — `-r` overrides it and `-r ''` lists the whole board, and `-l` is the task reads' comma-OR tag filter), `show` (goal, meta, member progress, and the members in canonical order — plus the box's **waiting** state: when every non-terminal member is done and a member parked in a due-tracked terminal lane (`waiting` on the shipped config; the done lane and `[due].ignore_lanes` never count) still has a due AHEAD, `ls`/`show`/`brief` print `⏳ waiting until <due> (<task>)` for the earliest such due and `--json` carries `waiting: {until, task}`, and revisit's `epic_all_done` stays quiet for it — a dateless parked member earns no such silence, since nobody could say what the box waits for), `set` (title/goal/meta/labels/repos), `activate` (enforces at most one active epic **per repo** — a clash is exit 2 naming the incumbent in `details.held`; `--reason` is recorded, with the timestamp, in the epic's body — the same body `furrow note <epic-id>` appends to — and `furrow sync` surfaces the switches it publishes), `deactivate`, `done` (stamps `closed` and clears `active` in the same write; deliberately never picks the next box — that judgment is the human's, and `lint`'s `epic-no-active` nags until someone decides. Members still open never **refuse** the close — closing a box ahead of its tail is untidy, not broken — they are **disclosed**: a stderr note names them (at most three ids, `+N more`) and `--json` carries `open_members`, an array of `{id, title, status, repeat}` (`[]` = none, `null` = the board could not be read). It counts exactly the set `lint` then warns `epic-closed` for (non-terminal members only), and a **repeating** member gets its own line, because that warn does not clear itself: closing an occurrence mints the next one, which inherits the epic, so the same warn returns under this closed box every cycle until one `furrow set <id> -e <open-epic>` moves the series to an open box. `deactivate` carries neither — it closes no box. Both verbs suggest the **previous active** box to return to: the open, currently-inactive one with the newest activation record, computed fresh from the activation log `activate` writes into each box's body (no stored "previous" pointer to go stale) — one `previous:` line plus a `previous` key in `--json` (null = unknown, the honest answer when no record decides it; records exist since v6, and stamps are minute-precision local time, fine for a suggestion the human confirms by running `epic activate` themselves)), `reopen` (clears `closed` — the CLI way back for a box closed by mistake; the box returns open and INACTIVE, so choosing it again stays a separate `epic activate`, and reopening an open box is exit 2), `rm` (deletes the box outright — see `rm` below: refused while members, waiting epics, or `[[e-…]]` links point at it, `--force` severs them, preview unless `--yes`), and `dep` (v7): `epic dep <epic> <dep-epic>...` makes a box WAIT ON others — "open this one after those close" — with the task-side `dep` contract (variadic, all-or-nothing, acyclic at write time, `--rm`, `--list` = both directions resolved to id+title+state). The edge is information, not enforcement: `activate` on a box with an open dep warns and proceeds (stderr note + `open_deps` beside the envelope), a dep on a closed epic is satisfied, `epic ls`/`brief` mark the waiting box (`open_deps` / ⏳ waits), `revisit` raises `epic_dep_done` when every dep is closed, and `lint` backstops merges (`epic-dep-cycle`, `epic-dep-missing`, `epic-dep-open`). Two more v7 declarations ride `set`: **`--standing`** marks a PERMANENT box (a mandate inbox, a parking lot) — exempt from the lifecycle nags `epic_all_done`/`epic_dep_done`/`epic_stuck`, and the box a review cadence exists for: once `furrow review <epic-ref>` has stamped it, `epic_review_due` fires when that review is older than `[review].stale_after_days` (v9), because a standing box is where a member's premise quietly rots and nothing else asks "is what's inside still true?" — and **`--pinned`** makes the box's actionable tasks lead `next`/`brief` regardless of the active scope (the pass-through channel; no repo slot needed, the tasks repo-filter themselves). Orthogonal to each other and to `activate` (mandate = standing+pinned, parking lot = standing only); clear with `--standing=false`/`--pinned=false`. Which boxes carry them is the operator's convention — furrow stays name-independent. A task's own membership is edited with `add -e` / `set -e` (`-e ''` unfiles).
...
- **`review`** — an id-shaped argument naming a real task stamps that task's `reviewed` timestamp (tracked apart from `updated`: a review changes no content); a full `owner/repo` or unique short name records a per-repo review clock; anything else is tried under the epic-ref contract and stamps that **box's** `reviewed` (v9) — the reset for `epic_review_due`, the STANDING box's review cadence on the same `[review].stale_after_days` clock as the repo nudge (a never-reviewed box stays quiet; the first review opts it in — exactly the repo clock's contract). `--by human` (default) advances the repo staleness-nudge clock; `--by agent` logs a sweep without advancing it, so an autonomous re-evaluation never stops furrow nudging a human (a task or epic carries one clock; the flag has no effect there).
...
- **`archive`** — with ids it retires exactly those (each must be in the done lane, else exit 2 — no stranding live work); with none it sweeps aged done **inheriting the board scope, like every read** (`-r` (repeatable) swaps that scope; `-r ''` sweeps the whole board and has to be typed — the widest blast radius costs the most keystrokes, not the fewest), `--older-than` adjusting the age guard (sweep-only flags; combining them with ids is exit 2). A task's attached media travels with it into `.furrow/archive/`, never orphaned in the hot store — and a file another hot body still shows (a repeat successor's, a box's) also STAYS in the hot store, said in the output (`assets: {copied, deleted, kept: [{name, held_by}]}` in `--json`): archive, unarchive, and rm share one rule, a store loses an asset only when nothing remaining there holds it. Previews unless `--yes`. Archiving is a **round trip** — see `unarchive`.
- **`unarchive`** — restores archived tasks to the hot board, all-or-nothing (a miss restores nothing, exits 1 with `details.missing`; an id already on the hot board is exit 2). The task comes back EXACTLY as archived — done lane, `closed` stamp, body and assets included — so restoring puts it back on the board and REOPENING stays `move`'s job (`furrow move <id> <lane>` clears `closed` on leaving done). `--json` is the batch mutators' envelope array (`before` is null, each envelope carries `unarchived: true`). No `--yes`: restoring destroys nothing. Every mutator's not-found error now names an archived id too (`details.archived` + "restore it with `furrow unarchive`"), so a write against a retired task steers to the fix instead of a bare miss.
- **`rm`** — deletes tasks outright: shard, body, and the attached assets nothing else still shows, gone (the board repo's git history is the only way back; an asset another body shows is kept and reported under `assets`, since deleting it would blank that body's picture for good). It is the **withdrawal of a filing**, not a retirement — `archive` folds DONE work into a round-trip store; `rm` is for the record that should not have been filed (everyday parking stays the icebox lane). Previews unless `--yes`; all-or-nothing (a miss removes nothing, exit 1 with `details.missing`, an archived id says to `unarchive` first). A target something still points at — another task's dep edge, a live `[[id]]` link in any body, and for a box its members and the epics whose deps name it — is **refused**: exit 2, kind `referenced`, every reference in `details.references` (`{deps, links, members, epic_deps}`). `--force` severs them instead: the dep edge is dropped, the link is de-linked to the bare id (the prose keeps its words), the member is unfiled, the epic dep is dropped — and none of that advances `updated` (bookkeeping, not progress, tidy's rule). References among the targets themselves never count, so a chain removes in one call. A target carrying a live `repeat` rule is its series' **only live** occurrence — a close hands the rule to the successor, a removal hands it to nobody — so removing it **ends the series**, and `rm` says so on the preview *and* on the `--yes` apply (`repeat: <id> carries a series (FREQ=DAILY, due …) — removing it ends the series; no successor is minted`). A disclosure, not a refusal: refusing is for a target something else still points at, and nothing afterwards records that the series existed (`lint` stays clean; git history is the only way back). The id is never reused (ids are random; nothing recycles a freed one). Guarded like every write by the session write guard — on the targets and on every entity `--force` edits — and the deletion is a furrow-owned change, so a plain `furrow sync` (or autocommit) publishes it; the hand `git rm` it replaces walked past the guard and needed its own push. `--json` is one report, `{dry_run, force, tasks, references}`. **`epic rm <epic>`** is the box twin (`{dry_run, force, epic, references}`); an unfiled member is then what `lint`'s `epic-required` names — refile it with `set -e`.
- **`tidy`** — the mechanized tidy pass: report, and with `--yes` prune, the two pieces of dead bookkeeping a hand-tidy keeps re-finding. `--done-deps` = dep edges from open tasks to done-lane tasks (satisfied edges gate nothing, but a board may keep them as history — epic slices wired as deps — so pruning is per-run opt-in, never config); `--unknown-keys` = the parked unknown keys across task/epic/repo shards and `meta.json` (what `upgrade` "kept for a human" and `unknown-shard-key` keeps naming — this is that human's tool, since hand-editing a machine-written shard is off the table; accepting it accepts that a *newer* binary's key is dropped with the junk). A bare `tidy` previews both classes; `--yes` requires naming what to apply. The apply never advances `updated` (a prune is bookkeeping, not progress — the reorder-respace rule), so the staleness clocks stay honest; the board repo's git history is the record of what was removed.
- **`lint`** — every finding carries a stable kebab-case `code` (`dangling-link`, `dep-cycle`, `epic-required`, `conflict-marker`, `unknown-shard-key`, `schema-outdated`, `archive-backlog`, …); branch on the code, not the message — the `id` field is contextual (a task id, an asset name, an `owner/repo`, `meta`, or `config`). Errors: dep cycles, a task sitting in an actionable (`[next].lanes`) lane with an unsatisfied dep (`ready-blocked` — `furrow next` will never hand it out, so the lane is lying: move the task back or drop the dep; a dep PARKED in a terminal non-done lane is named as such, because it never completes on its own — unpark it, drop the edge, or park this task too, but "wait" is not on the list; `brief`'s `blocked` band is the read-side view of the same set), a task whose promised instant has passed (`due-overdue` — board-wide, no epic scope, since dated work is usually parked outside the focus; the escape is the date itself, `set --due +1d` or `--clear-due`, not silence), an OPEN task with no epic once the board has any (`epic-required` — quiet on a board that never declared a box), two active epics naming the same repo (`epic-multi-active`), a done-lane task with no `closed`, a done-lane task still carrying a repeat rule its close should have handed on (`repeat-on-closed`), a body carrying git conflict markers (a half-merged progress record; `<<<<<<<`/`>>>>>>>` are flagged wherever they stand, fenced or not, since a conflict is exactly what splits a fence — only a bare `=======` inside a ``` fence is documentation and not flagged). Warns: a task promised for TODAY (`due-today`), an `updated` stamp ahead of the clock (`updated-in-future` — hidden from `is:stale`, `--since` and revisit until the next write rewinds it), two open tasks sharing a lane and a priority (`priority-duplicate` — their order is undefined; `reorder` places them), an open task under a closed epic (`epic-closed` — `epic done` discloses the same set as it closes; on a REPEATING member it returns every cycle, since the successor inherits the epic, until the series is re-filed), a repo with open work but no active epic (`epic-no-active`), dangling `[[id]]` links (archived ids are not dangling; `[[e-…]]` epic links are resolved too), reconcile gaps, asset hygiene (missing/orphan/≥5 MiB), a board whose layout is behind the binary, a file carrying keys this furrow doesn't know, a blank entry left in `labels`/`refs`/`repos`/`deps` by a binary that predates the write-time refusal (`blank-entry`), a `repeat_anchor` with no rule (`repeat-orphan-anchor` — inert inside furrow, but the published shard schema says the anchor is present iff the rule is, so a strict external reader is entitled to believe it), a label that names a repo — full `owner/repo` or a short name the board's repo universe resolves — where the first-class `repos` field belongs (`repo-as-label`; the read-side `-l` did-you-mean guard never second-guesses an existing label, so this warn is what keeps that guard honest), a done-lane task attached to no repo once the board uses repos at all (`done-draft` — a finished record no repo-scoped read can find; icebox/waiting drafts are a normal parked state and exempt), config clamp warnings, the `[lint].archive_done` backlog nudge, and — each only on a board that opted in — an open, non-terminal task whose body carries none of the board's `[lint].provenance_markers` (`provenance-missing`: the "where did this come from / how was it verified" pressure, in the board's own words; furrow ships no default vocabulary, so the check is off until a board declares one), a title carrying one of the board's `[lint].title_scope_markers` (`title-scope-marker` — the "scope narrowed in place" idiom, e.g. "(残り: docs only)": split the remainder out or retire the task; same no-default stance), and an open task sitting in the default (intake) lane past `[lint].stale_inbox_days` (`stale-inbox` — inbox-zero pressure, lane-bound and shorter than `[revisit].stale_days`; 0 = off). Narrow with `--code` (allow-list) / `--exclude-code` (deny-list; wins) / `--severity error|warn` — an unknown `--code` token is exit 2 with the vocabulary in `candidates`, while an unknown `[lint].ignore_codes` config entry only warns (clamp-don't-reject). A board can also RE-LEVEL one code with the `[lint.severity]` table (`due-overdue = "warn"` on a standalone board, where no CI consumes the red; promoting a warn it wants gating works the same way) — every consumer, this exit code and the `furrow sync` error count included, sees the effective level, and unlike `ignore_codes` the finding stays visible. **The filter drives the exit code**: a filtered-out problem is as if never found, so excluding or ignoring the last error exits 0 (the point — silence a permanently-dead check without reddening CI), and `--severity warn` always exits 0. `ignore_codes` is applied on every run, so the `furrow sync`/`brief` error count never reports an error `furrow lint` itself exits 0 on.
- **`migrate`** — dry-run by default (`--yes` applies); unmapped headings and `[[wikilink]]`s are reported, never dropped. `-l/--label` and `-e/--epic` are the one-flag-for-the-whole-batch stamps: `-e` files every imported task under one box (resolved before anything is written, so an unknown ref is exit 2 at the *preview* too), a bare import inherits the scope's single active box exactly as `add` does, and `-e ''` imports unfiled on purpose. An import that would land **open** tasks under no box on a board that HAS boxes is reported as one more migrate warning — the fact `lint` would otherwise raise as one `epic-required` error per task *after* the exit-0 import — counted over the non-terminal lanes only, so the number matches what lint reports.
...
furrow is **non-interactive by default** — it never prompts. Destructive operations are guarded: `archive`, `rm`, `epic rm`, `tidy`, `upgrade`, and `migrate` only preview unless you pass `--yes` — as does a `-q`/`-l`/`-r` selection on `set`/`done`/`move`.
...
whose members are all done (`epic_all_done` — quiet for a WAITING box, one whose only remaining members are parked with a due still ahead: `epic ls`/`epic show` say `⏳ waiting until <due> (<task>)`, and the date's own surfaces take over once it arrives), that hold open members with none
actionable (`epic_stuck`; standing boxes are exempt — deposits awaiting triage are their resting state), that are ACTIVE and untouched (`epic_stale`), or parked with every dep epic closed (`epic_dep_done` — its turn to open), STANDING boxes whose last review is past `[review].stale_after_days` (`epic_review_due` — re-check their contents; a never-reviewed box stays quiet), and repos whose review clock has lapsed
...
- **Working:** furrow is **CLI-only** — the core domain (`internal/core`) with
  ...
  field, first-class **epics** with epic-to-epic deps and a per-epic review clock, per-task **due dates**, and **recurrence** — a task that regenerates itself when closed (board layout v10 + the two-sided version gate: read-refuse a newer board,
