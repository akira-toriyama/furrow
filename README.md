...
actionable (`epic_stuck`), that are ACTIVE and untouched (`epic_stale`), or parked with every dep epic closed (`epic_dep_done` — its turn to open), STANDING boxes whose last review is past `[review].stale_after_days` (`epic_review_due` — re-check their contents; a never-reviewed box stays quiet), and repos whose review clock has lapsed
