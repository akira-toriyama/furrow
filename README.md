...
├── meta.json            # board-wide layout version {"schema_version": 9} — written ONLY by furrow, raised ONLY by `furrow upgrade`
├── tasks/
...
- **Working:** furrow is **CLI-only** — the core domain (`internal/core`) with
  ...
  field, first-class **epics** with epic-to-epic deps and a per-epic review clock, and per-task **due dates** (board layout v9 + the two-sided version gate: read-refuse a newer board,
...
- **Released:** tags are cut with GoReleaser → the Homebrew tap (see the
  ...
  since `v0.6.0`, layout v4 since `v0.8.0`, layout v5 since `v0.10.0`, layout v6 — the epic pivot — since `v1.0.0`, layout v7 — epic-to-epic deps + standing/pinned — since `v2.0.0`, layout v8 — per-task due dates — since `v3.0.0`, layout v9 — the per-epic review clock — since `v4.0.0`). The nix `flake.nix` carries a
