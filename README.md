...
| `check <id> [index]` | Mark a checklist item done by zero-based index (idempotent set, not a toggle; `--off` unchecks), or append one or more items (`--add` is repeatable; text is taken verbatim, commas included) | `--add <text>`, `--off` |
| `dep <id> <dep-id>...` | Add one or more dependencies (id waits on them) in a single write, or remove them with `--rm`; acyclic, idempotent, all-or-nothing (a bad dep-id aborts without a partial change) | `--rm` |
